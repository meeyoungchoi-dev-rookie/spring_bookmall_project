	...
	<table class="table text-center table-hover container">
		...
		<tbody>
			<tr>
				...
				<td><c:if
						test="${jangEntityDetail.allOrderDeleteStatus == false}">
					결제완료
				</c:if> <c:if test="${jangEntityDetail.allOrderDeleteStatus == true}">
					환불됨
				</c:if></td>
				<td><c:if
						test="${jangEntityDetail.allOrderDeleteStatus == false  && jangEntityDetail.productRefundstatus == false}">
						<form method="post" action="${path}/orders/refund">
							...
							<input type="hidden" name="orderSerialNum"
								value="${jangEntityDetail.orderSerialNum}"> <input
								type="hidden" name="orderSeq"
								value="${jangEntityDetail.orderSeq}"> <input
								type="hidden" name="bookOrderCnt"
								value="${jangEntityDetail.bookOrderCnt}"> <input
								type="hidden" name="bookSeq" value="${jangEntityDetail.bookSeq}">
							<input type="hidden" name="bookTitle"
								value="${jangEntityDetail.bookTitle}">
						</form>
					</c:if>
					<c:if
						test="${jangEntityDetail.allOrderDeleteStatus == true || jangEntityDetail.productRefundstatus == true}">
						환불완료
					</c:if>
...
<div class="container">
	<form method="post" action="${path}/mypage/search">
		<input type="hidden" name="myOrderSerialNum"
			value="${jangEntityDetail.orderSerialNum}">
